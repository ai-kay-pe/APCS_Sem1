...
public class Map {
	...
	*/
	public boolean move(int m){
		...
		if(m==2){
			if(posY==map.length-1) return false;
			else{
				// if(map[posX][posY]==3) map[posX][posY] = 1;
				// else 
				map[posX][posY] = 0;
		...
		if(m==4){
			if(posX==map[0].length-1) return false;
			else{
				// if(map[posX][posY]==3) map[posX][posY] = 1;
				// else 
				map[posX][posY] = 0;
		...
		if(m==6){
			if(posY==0||posX==map[0].length-1) return false;
			else{
				// if(map[posX][posY]==3) map[posX][posY] = 1;
				// else 
				map[posX][posY] = 0;
		...
		if(m==7){
			if(posY==map.length-1||posX==0) return false;
			else{ 
				// if(map[posX][posY]==3) map[posX][posY] = 1;
				// else
				map[posX][posY] = 0;
		...
		if(m==8){
			if(posY==map.length-1||posX==map[0].length-1) return false;
			else{
				// if(map[posX][posY]==3) map[posX][posY] = 1;
				// else
				map[posX][posY] = 0;
	...
	*/
	public boolean move(int row, int col){
		if(row>map.length-1||col>map[0].length-1) return false;
		else{
			if(map[posX][posY]==3) map[posX][posY] = 1;
			else map[posX][posY] = 0;
			posX = row;
			posY = col;
			map[posX][posY] = 2;
		}
	...
	*/
	public boolean dig(){
		if(posX==treasureX&&posY==treasureY) return true;	// Dummy value to make work
		else{
			map[posX][posY] = 1;
			return false;
		} 
